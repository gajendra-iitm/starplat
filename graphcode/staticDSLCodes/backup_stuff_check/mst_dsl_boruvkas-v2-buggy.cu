...
long long int mst_boruvkas(graph& g)
{
  ...
  cudaMemcpy(  d_weight,   h_weight, sizeof(int)*(E), cudaMemcpyHostToDevice);// #### MY ADDITION ###
  ...
  do{
    ...
    while(!finished) {
      ...
      cudaMemcpyToSymbol(::finished, &finished, sizeof(bool), 0, cudaMemcpyHostToDevice);
      mst_boruvkas_kernel_3<<<numBlocks, threadsPerBlock>>>(V,E,d_minweight,d_minedge,d_modified,d_parent);
      cudaDeviceSynchronize();
