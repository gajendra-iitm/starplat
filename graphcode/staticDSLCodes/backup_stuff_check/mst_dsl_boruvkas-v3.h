...
__global__ void mst_boruvkas_kernel_2(int V, int E, int* d_meta, int* d_data,int* d_minweight,bool* d_isCovered,int* d_parent,int* d_minedge, int* d_weight){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
    int dst = d_data[edge];
    int e = edge;
    if (d_parent[dst] != d_parent[v]){ // if filter begin 
      // if (d_isCovered[e] == false){ /// if filter begin redundant 
        int x = d_minweight[v]; // DEVICE ASSTMENT in .h

        if (x == -1 || x > d_weight[e]){ // if filter begin 
          d_minweight[v] = d_weight[e];
          d_minedge[v] = e;
...
__global__ void mst_boruvkas_kernel_3(int V, int E,int* d_minweight,int* d_minedge,bool* d_modified,bool* d_modified_next,int* d_parent){ // BEGIN KER FUN via ADDKERNEL
  ...
  // if (d_modified[v] == true){ // if filter begin 
    int par = d_parent[v]; // DEVICE ASSTMENT in .h   /// PANIC
    if(d_minedge[v]!=-1 ){
      if (d_minedge[par]==-1 || d_minweight[v] < d_minweight[par]){ // if filter begin 
  ...
  if (d_parent[v] == v){ // if filter begin 
    ...
    if (e != -1){ // if filter begin 
      ...
      if (epar != -1){ // if filter begin 
        ...
        // printf("e:%d,dst1:%d,v:%d,dstdstpar:%d,pardst1:%d \n",e,dst1,v,dstdstpar,pardst1);
