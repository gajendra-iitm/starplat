...
)
{
  ...
  //Reading kernel file
  FILE* kernelfp = fopen("preflow_push_V2.cl", "rb"); 
  size_t program_size;
  ...
  int excesstotal = 0; 
  int *h_visit = (int *)malloc(V*sizeof(int));
  cl_mem d_visit = clCreateBuffer(context,CL_MEM_READ_WRITE,(V)*sizeof(int),NULL, &status);
  ...
  do{
    ...
    status = clSetKernelArg(do_max_flow,5, sizeof(cl_mem),(void *)&d_weight);
    status = clSetKernelArg(do_max_flow,6, sizeof(cl_mem),(void *)&d_rev_meta);
    status = clSetKernelArg(do_max_flow, 7, sizeof(cl_mem), (void *)&d_excess);
    status = clSetKernelArg(do_max_flow, 8, sizeof(cl_mem), (void *)&d_source);
    status = clSetKernelArg(do_max_flow, 9, sizeof(cl_mem), (void *)&d_sink);
    status = clSetKernelArg(do_max_flow, 10, sizeof(cl_mem), (void *)&d_label);
    status = clSetKernelArg(do_max_flow, 11, sizeof(cl_mem), (void *)&d_residual_capacity);
    ...
    status = clSetKernelArg(do_max_flow,5, sizeof(cl_mem),(void *)&d_weight);
    status = clSetKernelArg(do_max_flow,6, sizeof(cl_mem),(void *)&d_rev_meta);
    status = clSetKernelArg(do_max_flow, 7, sizeof(cl_mem), (void *)&d_residual_capacity);
    status = clSetKernelArg(do_max_flow, 8, sizeof(cl_mem), (void *)&d_excess);
    status = clSetKernelArg(do_max_flow, 9, sizeof(cl_mem), (void *)&d_label);
    ...
    // Creating initvisit_kernel  Kernel
    cl_kernel initvisit_kernel = clCreateKernel(program, "initvisit_kernel", &status);

    // Initialization for visit variable
    int visitValue = (int)0; 
    status = clSetKernelArg(initvisit_kernel, 0 , sizeof(cl_mem), (void *)& d_visit);
    status = clSetKernelArg(initvisit_kernel, 1, sizeof(int) , (void*)& visitValue);
    status = clSetKernelArg(initvisit_kernel, 2, sizeof(int), (void*)&V);
    local_size = 128;
    global_size = (V + local_size -1)/ local_size * local_size;
    status = clEnqueueNDRangeKernel(command_queue, initvisit_kernel, 1, NULL, &global_size, &local_size, 0,NULL,&event);
    ...
    kernelTime = (double)(end-start)/convertToMS;
    totalTime = totalTime+ kernelTime;
    status = clReleaseKernel(initvisit_kernel);
    ...
    do{
      ...
      status = clEnqueueWriteBuffer(command_queue, d_finished, CL_TRUE, 0, sizeof(int),&finished , 0, NULL, NULL);
      ...
      status = clSetKernelArg(do_max_flow,6, sizeof(cl_mem),(void *)&d_rev_meta);
      status = clSetKernelArg(do_max_flow, 7, sizeof(cl_mem), (void *)&d_residual_capacity);
      status = clSetKernelArg(do_max_flow, 8, sizeof(cl_mem), (void *)&d_finished);
      status = clSetKernelArg(do_max_flow, 9, sizeof(cl_mem), (void *)&d_label);
      status = clSetKernelArg(do_max_flow, 10, sizeof(cl_mem), (void *)&d_visit);
    ...
    status = clSetKernelArg(do_max_flow,5, sizeof(cl_mem),(void *)&d_weight);
    status = clSetKernelArg(do_max_flow,6, sizeof(cl_mem),(void *)&d_rev_meta);
    status = clSetKernelArg(do_max_flow, 7, sizeof(cl_mem), (void *)&d_visit);
    status = clSetKernelArg(do_max_flow, 8, sizeof(cl_mem), (void *)&d_excesstotal);
  ...
  free(h_excess);
  status = clReleaseMemObject(d_visit);
  free(h_visit);
