...
int e = edge;
int ee = edge;
int ec = d_residual_capacity[e]; // asst in main

if (ec > 0 && d_label[v] > d_label[vv] + 1 && d_excess[v] > 0){ // if filter begin 
int d = ec; // asst in main
...
Q.submit([&](handler &h){ h.parallel_for(NUM_THREADS, [=](id<1> i){
for (; i < V; i += stride) d_visit[i] = (int)0;});
}).wait();

d_label[sink] = 0;
d_visit[sink] = 1;
int finished = 0; // asst in main

do{
finished = 0;
int *d_finished = malloc_device<int>(1, Q);
Q.submit([&](handler &h)
{ h.memcpy(d_finished, &finished, 1 * sizeof(int)); })
.wait();
...
atomic_ref<int, memory_order::relaxed, memory_scope::device, access::address_space::global_space> atomic_data(d_finished[0]);
atomic_data += 1;
...
Q.submit([&](handler &h)
{ h.memcpy(&finished, d_finished, 1 * sizeof(int)); })
.wait();
