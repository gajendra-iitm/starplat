...
auto dynamicAPFB_del(Graph& g, int previouslyMatched, EdgeProperty<bool>& modified, Updates * deleteBatch, 
      ...
      for( int u : g.get_bfs_nodes_for_phase(phase) )
      {
        ...
        for (int neigh : g.getNeighbors(u)) 
        {
          if (!visited.getValue(neigh) )
          {
            ...
            if (cmatch.getValue(neigh) == -1 && rmatch.getValue(neigh) == -1 )
            {
              unmatchedCount++;
            }
          }
...
auto dynamicAPFB_add(Graph& g, int previouslyMatched, EdgeProperty<bool>& modified, Updates * addBatch, 
  NodeProperty<int>& rmatch, NodeProperty<int>& cmatch, boost::mpi::communicator world )
{
  int matchCount = previouslyMatched;
  int evenComponents = 0;
  ...
  , boost::mpi::communicator world )
{
  ...
  while(updateBatch->nextBatch())
  {
    ...
    matchCount = dynamicAPFB_add(g,matchCount,modified_del,addBatch,rmatch,cmatch, world);
    ...
    matchCount = dynamicAPFB_del(g,matchCount,modified_add,deleteBatch,rmatch,cmatch, world);
