...
#ifndef GENCPP_LOCK-FREE-MAXFLOW-V3_1_H
#define GENCPP_LOCK-FREE-MAXFLOW-V3_1_H
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <cuda.h>
...
__global__ void do_max_flow_kernel_1(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_reverse_edge,int* d_residual_capacity,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void do_max_flow_kernel_5(int V, int E, int* d_meta, int* d_data, int* d_weight,int* d_residual_capacity,int* d_excess,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void do_max_flow_kernel_7(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_excess,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_residual_capacity,int* d_height){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void recalculate_max_flow_kernel_10(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_reverse_edge,int* d_residual_capacity,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
...
__global__ void recalculate_max_flow_kernel_14(int V, int E, int* d_meta, int* d_data, int* d_weight,int* d_residual_capacity,int* d_excess,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void recalculate_max_flow_kernel_16(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_excess,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_residual_capacity,int* d_height){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void recalculate_max_flow_kernel_19(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_excess){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void recalculate_max_flow_kernel_22(int V, int E, int* d_src, int* d_weight, int *d_rev_meta,int* d_reverse_edge,int* d_residual_capacity,int* d_excess,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void recalculate_max_flow_kernel_24(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_reverse_edge,int* d_residual_capacity,int* d_rev_residual_capacity,int* d_excess,int* d_height2){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] < 0 && v != source2 && v != sink2){ // if filter begin 
...
__global__ void recalculate_max_flow_kernel_27(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_excess,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_residual_capacity,int* d_height){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] > 0 && v != source2 && v != sink2){ // if filter begin 
