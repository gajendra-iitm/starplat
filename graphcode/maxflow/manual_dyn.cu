...
void Decremental( int V, int E, int* d_meta,int* d_data,int* d_src,int* d_weight,int* d_rev_meta, int source, int sink, int* d_residual_capacity, int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess,int* d_height2)
{
  ...
  decremental_kernel_13<<<numBlocks,  numThreads>>>(V,E,d_meta,d_data,d_src,d_weight,d_rev_meta,d_reverse_edge,d_rev_residual_capacity,d_excess);
  cudaDeviceSynchronize();
...
int recalculate_max_flow(graph& g, std::vector<update> updateBatch, int batchSize, int source, int sink)
{
