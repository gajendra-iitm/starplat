...
__global__ void rev_max_flow_kernel_1(int V, int E, int* d_meta, int* d_data,int* d_residual_capacity,int* d_reverse_edge, int * d_weight){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
