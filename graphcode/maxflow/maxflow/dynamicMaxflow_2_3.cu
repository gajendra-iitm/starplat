...

{
  ...
  do{
    ...
    staticMaxFlow_kernel_5<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_weight,d_rev_meta,d_reverse_edge,d_rev_residual_capacity,d_residual_capacity,d_excess,d_height);
    cudaDeviceSynchronize();
  ...
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
  ...
  initIndex<int><<<1,1>>>(V,d_height,source,(int)g.num_nodes()); //InitIndexDevice
  cudaMemcpyToSymbol(::source, &source, sizeof(int), 0, cudaMemcpyHostToDevice);
  incremental_kernel_8<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_weight,d_residual_capacity,d_excess,d_rev_residual_capacity);
  cudaDeviceSynchronize();
  cudaMemcpyFromSymbol(&source, ::source, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  do{
    ...
    incremental_kernel_10<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_weight,d_rev_meta,d_excess,d_reverse_edge,d_rev_residual_capacity,d_residual_capacity,d_height);
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(&source, ::source, sizeof(int), 0, cudaMemcpyDeviceToHost);
    cudaMemcpyFromSymbol(&sink, ::sink, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
  ...
  decremental_kernel_13<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_weight,d_rev_meta,d_reverse_edge,d_rev_residual_capacity,d_excess);
  cudaDeviceSynchronize();
  ...
  cudaMemcpyToSymbol(::sink, &sink, sizeof(int), 0, cudaMemcpyHostToDevice);
  decremental_kernel_16<<<numBlocks, threadsPerBlock>>>(V,E,d_src,d_weight,d_rev_meta,d_reverse_edge,d_excess,d_rev_residual_capacity,d_residual_capacity);
  cudaDeviceSynchronize();
  cudaMemcpyFromSymbol(&sink, ::sink, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  do{
    ...
    decremental_kernel_18<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_weight,d_rev_meta,d_residual_capacity,d_excess,d_reverse_edge,d_rev_residual_capacity,d_height2);
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(&source, ::source, sizeof(int), 0, cudaMemcpyDeviceToHost);
    cudaMemcpyFromSymbol(&sink, ::sink, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  int sink)
{
  ...
  for( int updateIndex = 0 ; updateIndex < updateBatch.size() ; updateIndex += batchSize){
    ...
    Decremental(g,source,sink,residual_capacity,rev_residual_capacity,reverse_edge,excess);
