...
#ifndef GENCPP_MANUAL_DYN_H
#define GENCPP_MANUAL_DYN_H
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <cuda.h>
...
__global__ void staticMaxFlow_kernel_3(int V, int E, int* d_meta, int* d_data, int* d_weight,int* d_residual_capacity,int* d_excess,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (v == source){ // if filter begin 
    // printf("push from src:%d\n",source);
    for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
      int vv = d_data[edge];
