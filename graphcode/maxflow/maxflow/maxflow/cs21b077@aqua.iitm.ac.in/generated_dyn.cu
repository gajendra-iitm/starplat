...

{
  ...
  do{
    ...
    staticMaxFlow_kernel_7<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_excess,d_reverse_edge,d_residual_capacity,d_height,d_rev_residual_capacity);
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(&source2, ::source2, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
  ...
  do{
    ...
    incremental_kernel_12<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_reverse_edge,d_rev_residual_capacity,d_residual_capacity,d_excess,d_height);
    cudaDeviceSynchronize();
  ...
  int sink)
{
  ...
  // NEED TO ADD CLEARING OF EXCESS AND OTHER PROPS!! I ADDED THE BELOW
  cudaFree(d_excess);
...
int  main( int  argc, char** argv) {
  ...
  G1.parseGraph();
   ...
   int reval = recalculate_max_flow(G1,updateEdges,batchSize,source,sink);
  cudaDeviceSynchronize();
  printf("REVAL VALUE:%d\n",reval);
  return 0;
}