...
#ifndef GENCPP_DYNAMICMAXFLOW_2_DYN_H
#define GENCPP_DYNAMICMAXFLOW_2_DYN_H
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <cuda.h>
...
__global__ void decremental_kernel_16(int V, int E, int* d_src, int *d_rev_meta,int* d_reverse_edge,int* d_excess,int* d_residual_capacity,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void decremental_kernel_18(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_excess,int* d_reverse_edge,int* d_residual_capacity,int* d_rev_residual_capacity,int* d_height2){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] < 0 && v != source && v != sink){ // if filter begin 
