  ...
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
  ...
  cudaMemcpyToSymbol(::sink, &sink, sizeof(int), 0, cudaMemcpyHostToDevice);
  decremental_kernel_16<<<numBlocks, threadsPerBlock>>>(V,E,d_src,d_rev_meta,d_reverse_edge,d_excess,d_rev_residual_capacity,d_residual_capacity);
  cudaDeviceSynchronize();
  cudaMemcpyFromSymbol(&sink, ::sink, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  do{
    ...
    decremental_kernel_18<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_residual_capacity,d_excess,d_reverse_edge,d_rev_residual_capacity,d_height2);
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(&source, ::source, sizeof(int), 0, cudaMemcpyDeviceToHost);
    cudaMemcpyFromSymbol(&sink, ::sink, sizeof(int), 0, cudaMemcpyDeviceToHost);
