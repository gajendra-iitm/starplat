...
void initializeLayers_cuda(GNN &gnn, vector<int> neuronsPerHiddenLayer, const char *transformation)
{
	vector<int> numFeaturesPerLayer;
	numFeaturesPerLayer.push_back(gnn.numFeatures());
	for (int i = 0; i < neuronsPerHiddenLayer.size(); i++)
	{
		numFeaturesPerLayer.push_back(neuronsPerHiddenLayer[i]);
	}
	numFeaturesPerLayer.push_back(gnn.numClasses());
	...
	initializeLayerFields<<<1, numFeaturesPerLayer.size()>>>(d_layers, numFeaturesPerLayer.size(), d_numFeaturesPerLayer, d_features, gnn.getGraph().num_nodes(), initialization_type);
	cudaDeviceSynchronize();
...
void predict_cuda(int num_classes, int num_nodes, int totalLayers)
{
...
int main()
{
	...
	vector<int> neuronsPerHiddenLayer = {8};//sample graph
	...
	graph G("./sample_graphs/sample_graph/sample_graph.txt");
	G.parseGraph();
	...
	GNN gnn(G, "./sample_graphs/sample_graph/sample_graph_feat.txt", "./sample_graphs/sample_graph/sample_graph_labels.txt");
	gnn.gcnPreprocessing();
	cout << "preprosessing done" << endl;

	initializeLayers_cuda(gnn, neuronsPerHiddenLayer, gnn.getGraph().num_nodes(), transformation);
	cudaDeviceSynchronize();
	...
	for (int epoch = 1; epoch < 50; epoch++)
	{
		...
		// exit(0);
		predict_cuda(gnn.numClasses(), num_nodes, numFeaturesPerLayer.size());
	}
