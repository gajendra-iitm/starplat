...

{
  ...
  mst_boruvkas_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_parent,d_isCovered,d_destination);
  ...
  do{
    ...
    mst_boruvkas_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_minweight,d_isCovered,d_parent,d_minedge);
    cudaDeviceSynchronize();
    ...
    while(!finished) {
      ...
      cudaMemcpyToSymbol(::finished, &finished, sizeof(bool), 0, cudaMemcpyHostToDevice);
      mst_boruvkas_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_minweight,d_minedge,d_parent);
      cudaDeviceSynchronize();
