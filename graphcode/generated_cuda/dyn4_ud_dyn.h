...
#ifndef GENCPP_DYN4_UD_H
#define GENCPP_DYN4_UD_H
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <cuda.h>
...
__global__ void decremental_kernel_22(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_excess,int* d_residual_capacity,int* d_height,int* d_parallel_edge,int* d_reverse_edge,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] < 0 && v != source && v != sink && d_height[v] < V){ // if filter begin 
...
__global__ void recalculate_max_flow_kernel_26(int V, int E, int* d_meta, int* d_data,int* d_rev_residual_capacity,int* d_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
...
__global__ void recalculate_max_flow_kernel_28(int V, int E,int* d_excess){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
