...
__global__ void APFB_kernel(int V, int E,int* d_cmatch,int* d_bfsArray){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
...
__global__ void APFB_kernel(int V, int E, int* d_meta, int* d_data,int* d_bfsArray,int* d_rmatch,int* d_predeccesor){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void APFB_kernel(int V, int E,int* d_rmatch,bool* d_compress){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void APFB_kernel(int V, int E,bool* d_compress,int* d_predeccesor,int* d_cmatch,bool* d_compress_next,int* d_rmatch){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void APFB_kernel(int V, int E,bool* d_compress_next,bool* d_compress){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (row_vertex >= nc){ // if filter begin 
...
__global__ void APFB_kernel(int V, int E,int* d_rmatch,int* d_cmatch){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
