...

{
  ...
  do{
    ...
    staticMaxFlow_kernel_5<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_reverse_edge,d_rev_residual_capacity,d_residual_capacity,d_excess,d_height);
    cudaDeviceSynchronize();
...
__device__ int sink ;
void Incremental(graph& g, int V,int E,int* d_meta,int* d_data,int* d_rev_meta,int* d_src,int* d_weight, int source, int sink, int* d_residual_capacity, 
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
