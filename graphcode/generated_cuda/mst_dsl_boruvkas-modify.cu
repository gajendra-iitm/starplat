...

{
  ...
  mst_boruvkas_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_isCovered,d_destination,d_parent);
  ...
  do{
    ...
    while(!finished) {
      ...
      cudaMemcpyToSymbol(::finished, &finished, sizeof(bool), 0, cudaMemcpyHostToDevice);
      mst_boruvkas_kernel<<<numBlocks, threadsPerBlock>>>(V,E,d_minedge,d_minweight,d_modified,d_parent);
      cudaDeviceSynchronize();
