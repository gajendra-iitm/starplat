...

{
  ...
  do{
    ...
    staticMaxFlow_kernel_7<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_reverse_edge,d_rev_residual_capacity,d_residual_capacity,d_height,d_excess);
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(&source2, ::source2, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
  ...
  do{
    ...
    incremental_kernel_12<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_rev_residual_capacity,d_residual_capacity,d_excess,d_height,d_reverse_edge);
    cudaDeviceSynchronize();
    cudaMemcpyFromSymbol(&source0, ::source0, sizeof(int), 0, cudaMemcpyDeviceToHost);
  ...
  int* d_rev_residual_capacity, int* d_reverse_edge, int* d_excess)
{
  ...
  decremental_kernel_15<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_rev_residual_capacity,d_reverse_edge,d_excess);
  cudaDeviceSynchronize();
  ...
  do{
    ...
    decremental_kernel_20<<<numBlocks, threadsPerBlock>>>(V,E,d_meta,d_data,d_src,d_rev_meta,d_excess,d_residual_capacity,d_rev_residual_capacity,d_height2,d_reverse_edge);
    cudaDeviceSynchronize();
  ...
  int sink)
{
  ...
  cudaMemcpy(d_weight,h_weight,sizeof(int)*(E),cudaMemcpyHostToDevice);
  cudaMemcpy(d_rev_meta,h_rev_meta,sizeof(int)*(V+1),cudaMemcpyHostToDevice);
  unsigned threadsPerBlock   = (V < THREADS_PER_BLOCK)? THREADS_PER_BLOCK: V;
  ...
  staticMaxFlow(g,source,sink,d_residual_capacity,d_rev_residual_capacity,d_reverse_edge,d_excess);
  ...
  for( int updateIndex = 0 ; updateIndex < updateBatch.size() ; updateIndex += _batchSize){
    ...
    Decremental(g,V,E,d_meta,d_data,d_rev_meta,d_src,d_weight,source,sink,d_residual_capacity,d_rev_residual_capacity,d_reverse_edge,d_excess);

    Incremental(g,V,E,d_meta,d_data,d_rev_meta,d_src,d_weight,source,sink,d_residual_capacity,d_rev_residual_capacity,d_reverse_edge,d_excess);
