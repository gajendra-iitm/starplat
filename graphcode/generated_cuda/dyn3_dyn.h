...
#ifndef GENCPP_DYN3_H
#define GENCPP_DYN3_H
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <cuda.h>
...
__device__ int kernel ; // DEVICE ASSTMENT in .h

__global__ void staticMaxFlow_kernel_7(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_residual_capacity,int* d_height,int* d_excess){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void incremental_kernel_12(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_rev_residual_capacity,int* d_residual_capacity,int* d_excess,int* d_height,int* d_reverse_edge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void decremental_kernel_15(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_rev_residual_capacity,int* d_reverse_edge,int* d_excess){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void decremental_kernel_20(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_excess,int* d_residual_capacity,int* d_rev_residual_capacity,int* d_height2,int* d_reverse_edge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
