...
__global__ void mst_boruvkas_kernel(int V, int E,int* d_minweight,int* d_minedge,bool* d_modified,bool* d_modified_next,int* d_parent){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
