...
__global__ void Boruvka_kernel(int V, int E,int* d_color,int* d_nodeId){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
...
__global__ void Boruvka_kernel(int V, int E, int* d_meta, int* d_data,int* d_color,int* d_minEdge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void Boruvka_kernel(int V, int E,int* d_color,int* d_minEdge,int* d_minEdgeOfComp){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void Boruvka_kernel(int V, int E,int* d_color,int* d_nodeId,int* d_minEdgeOfComp){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void Boruvka_kernel(int V, int E,int* d_color,int* d_nodeId,int* d_minEdgeOfComp,bool* d_isMSTEdge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_color[src] == d_nodeId[src]){ // if filter begin 
...
__global__ void Boruvka_kernel(int V, int E,int* d_color){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
