...
#ifndef GENCPP_DYN4_UD_SIMPLE_NOBFS_H
#define GENCPP_DYN4_UD_SIMPLE_NOBFS_H
#include <stdio.h>
#include <stdlib.h>
#include <limits.h>
#include <cuda.h>
...
__device__ int kernel_cycles0 ;


__device__ int num_nodes ; // DEVICE ASSTMENT in .h
...
__global__ void staticMaxFlow_kernel_10(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_residual_capacity,int* d_excess,int* d_height,int* d_rev_residual_capacity,int* d_parallel_edge,int* d_reverse_edge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] > 0 && v != source0 && v != sink0 && d_height[v] < num_nodes){ // if filter begin 
    flag1 = true;
    int cycle = kernel_cycles0; // DEVICE ASSTMENT in .h
...
__device__ int kernel_cycles1 ;
__device__ int num_nodes1 ; // DEVICE ASSTMENT in .h

__global__ void incremental_kernel_13(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_reverse_edge,int* d_residual_capacity,int* d_excess,int* d_rev_residual_capacity,int* d_parallel_edge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (v == source1){ // if filter begin 
    for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
      int vv = d_data[edge];
      int forward_edge = edge;
      int d = d_residual_capacity[forward_edge]; // DEVICE ASSTMENT in .h
...
__global__ void incremental_kernel_16(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_excess,int* d_height,int* d_parallel_edge,int* d_residual_capacity,int* d_reverse_edge,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] > 0 && v != source1 && v != sink1 && d_height[v] < num_nodes1){ // if filter begin 
    flag2 = true;
    int cycle = kernel_cycles1; // DEVICE ASSTMENT in .h
...
__device__ int kernel_cycles2 ;
__device__ int num_nodes2 ; // DEVICE ASSTMENT in .h

__global__ void decremental_kernel_19(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_excess,int* d_residual_capacity,int* d_parallel_edge){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
...
__global__ void decremental_kernel_22(int V, int E, int* d_meta, int* d_data, int* d_src, int *d_rev_meta,int* d_reverse_edge,int* d_excess,int* d_residual_capacity,int* d_height,int* d_parallel_edge,int* d_rev_residual_capacity){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  if (d_excess[v] < 0 && v != source2 && v != sink2 && d_height[v] < num_nodes2){ // if filter begin 
    flag3 = true;
    int cycle = kernel_cycles2; // DEVICE ASSTMENT in .h
...
__device__ int batchSize ;
__device__ int source ;
__device__ int cycle ;
__device__ int sink ; // DEVICE ASSTMENT in .h
...
__global__ void recalculate_max_flow_kernel_26(int V, int E, int* d_meta, int* d_data, int* d_src, int* d_weight, int *d_rev_meta,int* d_reverse_edge,int* d_rev_residual_capacity,int* d_residual_capacity,int* d_excess){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
  for (int edge = d_meta[v]; edge < d_meta[v+1]; edge++) { // FOR NBR ITR 
...
__global__ void recalculate_max_flow_kernel_30(int V, int E,int* d_excess){ // BEGIN KER FUN via ADDKERNEL
  float num_nodes  = V;
  unsigned v = blockIdx.x * blockDim.x + threadIdx.x;
  if(v >= V) return;
