...
namespace spcuda
{
  ...
  void dsl_cpp_generator::generation_begin()
  {
    ...
    header.pushString("#include ");
    addIncludeToFile("thrust/host_vector.h", header, true);
    header.pushString("#include ");
  ...
  void dsl_cpp_generator::generateStatement(statement *stmt, bool isMainFile)
  {
    ...
    {
      cout << "Generating variable declaration\n";
      generateVariableDecl((declaration *)stmt, isMainFile);
      targetFile.pushstr_newL(";");
    }
    if (stmt->getTypeofNode() == NODE_ASSIGN)
    {
      // generateAssignmentStmt((assignment*)stmt, true);
                                           ...
                                           bool isMainFile)
  {
    ...
    // cout<<"INSIDE PROCCALL OF GENERATION "<<proc_callStmt<<"\n";
    string methodID(procedure->getMethodId()->getIdentifier());
    string nodeCall("attachNodeProperty");
    string edgeCall("attachEdgeProperty");
    int nodePropComp = methodID.compare(nodeCall);
    int edgePropComp = methodID.compare(edgeCall);
    if (nodePropComp == 0)
    {
      //~ char strBuffer[1024];
                                               ...
                                               bool isMainFile)
  {
    ...
    else if (type->isPrimitiveType())
    {
      ...
      */
      ...
      if (declStmt->isInitialized())
      {
        ...
        generateExpr(declStmt->getExpressionAssigned(), isMainFile, false, true); // PRINTS RHS? YES
      ...
      //  // targetFile.pushstr_newL(";");
      // }*/
      /* if(declInHeader && declStmt->getExpressionAssigned()->getExpressionFamily() != EXPR_PROCCALL && !flag) { //Added flag to prevent the generation of unnecessary semicolons for device variables
    ...
    else if (type->gettypeId() == TYPE_CONTAINER)
    {
      targetFile.pushstr_space(convertToCppType(type));
      targetFile.pushString(declStmt->getdeclId()->getIdentifier());
      // cout<<"Testing container   "<<declStmt->getdeclId()->getIdentifier()<<endl;
                                               ...
                                               bool isMainFile)
  {
    ...
    case EXPR_STRINGCONSTANT:
      targetFile.pushString(expr->getStringConstant());
      break;
    default:
      assert(false);
  ...
  void dsl_cpp_generator::generateArgList(list<argument *> argList, bool addBraces, bool isMainFile)
  {
    ...
    // cout<<"Argument length = "<<argList.size()<<endl;
    int commaCounts = 0;
    list<argument *>::iterator itr;
    for (itr = argList.begin(); itr != argList.end(); itr++)
    {
  ...
  void dsl_cpp_generator::generateExpr(Expression *expr, bool isMainFile, bool isAtomic, bool isSecondArg)
  { // isAtomic default to false
    ...
    else if (expr->isProcCallExpr())
    {
      ...
      generate_exprProcCall(expr, isMainFile, isSecondArg);
    }
  ...
  void dsl_cpp_generator::generate_exprProcCall(Expression *expr,
                                                bool isMainFile, bool isSecondArg)
  {
    ...
    else
    {
      ...
      else if (indexExpr != 0)
      {
        ...
        cout << "proccalll data  " << getProcName(proc).data() << endl;
        targetFile.pushString(getProcName(proc).data());
      }
      else
      {
        targetFile.pushString(getProcName(proc).data());
  ...
  **/
  ...
  const char *dsl_cpp_generator::convertToCppType(Type *type)
  {
    if (type->isPrimitiveType())
    {
      ...
      case TYPE_EDGE:
        return "int";
      case TYPE_STRING:
        return "thrust::device_vector<char> ";
      default:
        assert(false);
      }
    }
  ...
  void dsl_cpp_generator::generateFuncBody(Function *proc, bool isMainFile)
  {
    ...
    if (genCSR)
    {
      targetFile.pushstr_newL("// CSR BEGIN");
      ...
      targetFile.pushstr_newL("// CSR END");

      targetFile.pushString("//LAUNCH CONFIG");

      generateLaunchConfig("nodes"); // Most of the time unless we perform edgeBased!

      targetFile.NewLine();

      targetFile.pushstr_newL("// TIMER START");
      generateStartTimer();
    }
  }
