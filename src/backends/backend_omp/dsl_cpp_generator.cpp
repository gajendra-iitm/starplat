...
namespace spomp
{
    ...
    else
    {
      ...
      if (sourceId != NULL)
      {
        if (sourceId->getSymbolInfo()->getType()->gettypeId() == TYPE_SETN)
        {
          ...
          main.pushstr_newL("set<int>::iterator itr;");
  ...
  void dsl_cpp_generator::generate_exprLiteral(Expression *expr)
  {
    ...
    case EXPR_STRINGCONSTANT:
      // printf("\nSOMETHINGSOMETHING\t%s\n", expr->getStringConstant());
      main.pushString(expr->getStringConstant());
      cout << "This is  smething that we are figuring out \t" << expr->getStringConstant().size() << endl;
      break;
    default:
      assert(false);
  ...
  void dsl_cpp_generator::generateArgList(list<argument *> argList, bool addBraces)
  {
    ...
    for (itr = argList.begin(); itr != argList.end(); itr++)
    {
      ...
      cout << "expr  " << arg->getExpr()->getStringConstant() << endl;
      // main.pushString(strBuffer);
      generateExpr(expr);
  ...
  const char *dsl_cpp_generator::convertToCppType(Type *type)
  {
    ...
    else if (type->isCollectionType())
    {
      ...
      case TYPE_CONTAINER:
      {
        ...
        vecString = vecString + ">";
        // cout<<"This is container type test  "<<vecString<<endl;
